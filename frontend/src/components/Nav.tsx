      ...
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className=''>
          {!profile?.success ? <button
            ...
            :
            <button className='bg-chinese-black flex gap-3 text-white px-6 py-2 rounded transition duration-300 outline-cadet-gray  outline-1 hover:outline '>
              {profile?.data.username}
            <img className='rounded-full' src={profile?.data.avatar} width={24} />
            </button>}
        </div>
      </div>
    </nav>
